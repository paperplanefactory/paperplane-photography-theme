...
while (have_posts()):
  ...
  function prev_att_link($att_post = null)
  {
    ...
    if ($ppid != false) {
      return '<a href="' . get_attachment_link($ppid) . '" class="previous-attachment-link data-swup-preload" aria-label="' . __('Previous image', 'paperplane-photography-theme') . '" class="data-swup-preload"></a>';
    } else {
  ...
  function next_att_link($att_post = null)
  {
    ...
    if ($ppid != false) {
      return '<a href="' . get_attachment_link($ppid) . '" class="next-attachment-link data-swup-preload" aria-label="' . __('Next image', 'paperplane-photography-theme') . '" class="data-swup-preload"></a>';
    } else {
      return false;
    }
  }
  ...
  <div id="delight-approved" class="photo-frame" data-picture-id="<?php echo get_the_ID(); ?>">
    ...
    <div class="navi-click navi-click-left delight-area">
      <?php echo prev_att_link(); ?>
    </div>
    <div class="navi-click navi-click-right delight-area">
      <?php echo next_att_link(); ?>
    </div>
  </div>
      ...
      <div class="wrapper-padded-more-650 delight-area">
        <div class="content-styled aligncenter">
          ...
          <div class="alignleft">
            ...
            <?php foreach ($original_post as $post):
              setup_postdata($post); ?>
              <?php the_content(); ?>
            <?php endforeach;
            wp_reset_query(); ?>
