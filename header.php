...
<head>
	...
	<style>
		body {
			font-family:
				<?php the_field( 'body_font_family', 'options' ); ?>
			;
			font-weight:
				<?php the_field( 'body_font_weight', 'options' ); ?>
			;
		}
		...
		.as-h4 {
			font-family:
				<?php the_field( 'titles_font_family', 'options' ); ?>
			;
			font-weight:
				<?php the_field( 'titles_font_weight', 'options' ); ?>
			;
		}
		...
		.content-styled ol {
			font-family:
				<?php the_field( 'paragraphs_font_family', 'options' ); ?>
			;
			font-weight:
				<?php the_field( 'paragraphs_font_weight', 'options' ); ?>
			;
		}
		...
		.menu p {
			font-family:
				<?php the_field( 'menu_font_family', 'options' ); ?>
			;
			font-weight:
				<?php the_field( 'menu_font_weight', 'options' ); ?>
			;
		}

		.navi-text {
			font-family:
				<?php the_field( 'navitext_font_family', 'options' ); ?>
			;
			font-weight:
				<?php the_field( 'navitext_font_weight', 'options' ); ?>
			;
		}

		.cta-1 {
			font-family:
				<?php the_field( 'cta_font_family', 'options' ); ?>
			;
			font-weight:
				<?php the_field( 'cta_font_weight', 'options' ); ?>
			;
		}
		...
		.wp-caption-text {
			font-family:
				<?php the_field( 'inputs_font_family', 'options' ); ?>
			;
			font-weight:
				<?php the_field( 'inputs_font_weight', 'options' ); ?>
			;
		}
		...
		if ( $custom_colors == 1 ) :
			...
			body.clear-theme {
				color:
					<?php the_field( 'custom_color_5', 'options' ); ?>
				;
				background-color:
			...
			.clear-theme textarea {
				border-color:
					<?php the_field( 'custom_color_3', 'options' ); ?>
					!important;
			}
			...
			.clear-theme .sub-menu a {
				color:
					<?php the_field( 'custom_color_5', 'options' ); ?>
					!important;
			}

			.clear-theme .sub-menu .current-menu-item a,
			.clear-theme .sub-menu .current-page-ancestor a {
				color:
					<?php the_field( 'custom_color_3', 'options' ); ?>
					!important;
			}
			...
			.clear-theme .navi-info a:hover {
				color:
					<?php the_field( 'custom_color_4', 'options' ); ?>
				;
			}
			...
			.clear-theme .txt-4:link,
			.clear-theme .txt-4:visited,
			.clear-theme .video-slide .slick-next,
			.clear-theme .video-slide .prev-next {
				color:
					<?php the_field( 'custom_color_4', 'options' ); ?>
				;
			}
	...
	<meta name="theme-color" content="#DCDCDC" class="get-custom-tile-color-js"
		data-custom-color="<?php echo $tile_color; ?>">
	<!-- Windows Phone -->
...
<body class="" data-ga-measurement-id="<?php the_field( 'google_analytics_measurement_id', 'options' ); ?>">
	<div class="eye de-highlight only-desktop">
		<span class="highlight all-pointer-events pointered" title="highlight this picture"><i
			...
			<div id="header-structure">
				...
				<nav class="swupped menu <?php echo $fullwidth_menu_class; ?>">
						...
						<ul class="navi-info swupped-link">
							...
							<li class="buy-button <?php echo $buy_button_status; ?>">
								<form action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_blank"
									class="delight-area pay-picture">
									<input type="hidden" name="cmd" value="_xclick">
									<input type="hidden" name="business"
										value="<?php the_field( 'indirizzo_email_paypal', 'option' ); ?>">
									...
									<input type="hidden" name="currency_code"
										value="<?php the_field( 'tipo_di_valuta', 'option' ); ?>">
									<input type="hidden" name="button_subtype" value="services">
									<input type="hidden" name="no_note" value="0">
									<input type="hidden" name="tax_rate" value="0">
									<input type="hidden" name="shipping"
										value="<?php the_field( 'costi_di_spedizione', 'option' ); ?>">
									<input type="hidden" name="bn" value="PP-BuyNowBF:btn_buynowCC_LG.gif:NonHostedGuest">
									<button type="submit" title="Buy this picture"
										aria-label="<?php _e( 'Buy this picture', 'paperplane-photography-theme' ); ?>"><i
											class="icon-credit-card-alt"></i></button>
								</form>
							</li>
							<?php if ( $torna_elenco === 'si' ) : ?>
								<li>
									...
									<a href="<?php echo $first_category_url; ?>"
										title="<?php _e( 'back to list', 'paperplane-photography-theme' ); ?>"><i
											class="icon-back delight-area"></i></a>
								</li>
							<?php endif; ?>
							<?php if ( $evidenziatore_foto === 'si' ) : ?>
								<li>
									<span class="highlight all-pointer-events pointered"
										title="<?php _e( 'highlight this picture', 'paperplane-photography-theme' ); ?>"><i
											class="icon-eye"></i></span>
								</li>
							<?php endif; ?>
							<?php if ( $show_play_pause_button === 'si' && $image_gallery_system != 'scrollpage' ) : ?>
								<li>
									<span class="play-pause delight-area pointered"
										title="<?php _e( 'play/pause gallery', 'paperplane-photography-theme' ); ?>"
										player-attribute="pause"><i class="play-pauser icon-play"></i></span>
								</li>
							<?php endif; ?>
							<?php if ( $elenco_foto === 'si' && $image_gallery_system != 'scrollpage' ) : ?>
								<li>
									<span class="list thumb-list delight-area pointered"
										title="<?php _e( 'view images list', 'paperplane-photography-theme' ); ?>"><i
											class="icon-js icon-th-large-outline"></i></span>
								</li>
							<?php endif; ?>
						</ul>
					<?php endif; ?>
					<?php if ( is_singular( 'news' ) ) : ?>
						<ul class="navi-info swupped-link">
							<li>
								<a href="<?php the_field( 'news_archive', 'option' ) ?>" class="delight-area"
									title="<?php _e( 'back to list', 'paperplane-photography-theme' ); ?>"><i
										class="icon-back"></i></a>
							</li>
						</ul>
					...
					<?php if ( is_singular( 'book' ) ) : ?>
						<ul class="navi-info swupped-link">
							<li>
								<a href="<?php the_field( 'books_archive', 'option' ) ?>" class="delight-area"
									title="<?php _e( 'back to list', 'paperplane-photography-theme' ); ?>"><i
										class="icon-back"></i></a>
							</li>
						</ul>
				...
				<div class="hamburger delight-area <?php echo $show_hamburger_button_desktop_class; ?>">
					<div type="button" aria-haspopup="true" aria-expanded="false" aria-controls="menu" aria-label="menu"
					...
					<?php if ( $show_search_form === 'yes' ) : ?>
						<div class="form-hold search-hold">
							<form action="<?php echo home_url(); ?>/?" method="get" id="sbk-search">
								<input type="text" class="search-kw" name="s" id="keyword"
									placeholder="Cosa stai cercando?" />
								<button type="submit" class="submitter"
									aria-label="<?php _e( 'Search site', 'paperplane-photography-theme' ); ?>"><i
