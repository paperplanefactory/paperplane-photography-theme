...
function redirect_if_not_post_attachment()
{
  ...
  if ($post) {
    ...
    if (($image_gallery_system === 'slider' && is_attachment()) || ($image_gallery_system === 'scrollpage' && is_attachment())) {
      $parent_url = get_permalink($parent);
      wp_redirect($parent_url, 301);
      exit;
    }
...
function paperplane_images_add_meta_tags()
{
  global $post;
  if ($post) {
    $this_family = get_post_type($post->ID);
    $parent = get_post_field('post_parent', $post->ID);
    $parent_family = get_post_type($parent);
    $image_gallery_system = get_field('image_gallery_system', $post->ID);
    if ($this_family === 'post' && $image_gallery_system === 'one-page-one-picture') {
      $thumb_id = get_post_thumbnail_id($post->ID);
      if ($thumb_id != 0) {
        $thumb_url_desktop = wp_get_attachment_image_src($thumb_id, 'single_image_retina', true);
        $thumb_url_desktop = $thumb_url_desktop[0];
        $thumb_url_mobile = wp_get_attachment_image_src($thumb_id, 'single_image', true);
        $thumb_url_mobile = $thumb_url_mobile[0];
        echo '<link rel="preload" href="' . $thumb_url_mobile . '" as="image" media="(max-width: 1024px)" />' . "\n";
        echo '<link rel="preload" href="' . $thumb_url_desktop . '" as="image" media="(min-width: 1025px)" />' . "\n";
      }
    }

    if ($this_family === 'post' && $image_gallery_system === 'scrollpage') {
      $repeater = get_field('slides_manager_repeater', $post->ID);
      $slides_manager_repeater_content_type = $repeater[0]['slides_manager_repeater_content_type'];
      if ($slides_manager_repeater_content_type === 'picture') {
        $slides_manager_repeater_picture = $repeater[0]['slides_manager_repeater_picture']['ID'];
        $thumb_url_desktop = wp_get_attachment_image_src($slides_manager_repeater_picture, 'single_image_retina', true);
        $thumb_url_desktop = $thumb_url_desktop[0];
        $thumb_url_mobile = wp_get_attachment_image_src($slides_manager_repeater_picture, 'single_image', true);
        $thumb_url_mobile = $thumb_url_mobile[0];
        echo '<link rel="preload" href="' . $thumb_url_mobile . '" as="image" media="(max-width: 1024px)" />' . "\n";
        echo '<link rel="preload" href="' . $thumb_url_desktop . '" as="image" media="(min-width: 1025px)" />' . "\n";
      }
